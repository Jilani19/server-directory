...
async function validate() {
  ...
  // 1. API Validation
  const resBase = await axios.get(BASE_URL);
  fs.writeFileSync(path.join(OUTPUT_DIR, 'API_VALIDATION.md'), 
  ...
  const p1 = await axios.get(BASE_URL + "?page=1&limit=25");
  const p2 = await axios.get(BASE_URL + "?page=2&limit=25");
  const p3 = await axios.get(BASE_URL + "?page=3&limit=25");
  const p4 = await axios.get(BASE_URL + "?page=4&limit=25");

  fs.writeFileSync(path.join(OUTPUT_DIR, 'PAGINATION_VALIDATION.md'), 
  ...
  // 3. Search Validation
  const s1 = await axios.get(BASE_URL + "?search=pfizer");
  const s2 = await axios.get(BASE_URL + "?search=JNJ");

  fs.writeFileSync(path.join(OUTPUT_DIR, 'SEARCH_VALIDATION.md'), 
    "# SEARCH VALIDATION\n\n" +
  ...
  // 4. Filter Validation
  const f1 = await axios.get(BASE_URL + "?companyType=Biotechnology");
  const f2 = await axios.get(BASE_URL + "?companyType=Vaccines");

  fs.writeFileSync(path.join(OUTPUT_DIR, 'FILTER_VALIDATION.md'), 
    "# FILTER VALIDATION\n\n" +
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'DIRECTORY_VALIDATION.md'), 
