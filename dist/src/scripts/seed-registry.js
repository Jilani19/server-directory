...
async function seedRegistry() {
    ...
    await prisma_1.prisma.country.deleteMany({});
    await prisma_1.prisma.user.deleteMany({});
    await prisma_1.prisma.role.deleteMany({});
    const adminRole = await prisma_1.prisma.role.create({ data: { id: "admin-role", name: "Admin" } });
    const systemUser = await prisma_1.prisma.user.create({ data: { id: "system", email: "system@localhost", passwordHash: "none", roleId: adminRole.id } });
    // Seed the strictly allowed countries
    const usa = await prisma_1.prisma.country.create({ data: { id: (0, uuid_1.v4)(), name: "United States", code: "US" } });
    const india = await prisma_1.prisma.country.create({ data: { id: (0, uuid_1.v4)(), name: "India", code: "IN" } });
    ...
    for (let i = 0; i < COMBINED_COMPANIES.length; i++) {
        ...
        await prisma_1.prisma.company.create({
            data: {
                id: (0, uuid_1.v4)(),
                name,
                slug,
