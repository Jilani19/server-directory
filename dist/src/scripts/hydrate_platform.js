...
class ClinicalTrialsWorker extends BaseWorker {
    ...
    async execute(company) {
        ...
        try {
            const res = await axios_1.default.get(`https://clinicaltrials.gov/api/v2/studies?query.term=${encodeURIComponent(company.name)}&pageSize=5`);
...
class PublicationWorker extends BaseWorker {
    ...
    async execute(company) {
        super.execute(company);
        try {
            const searchRes = await axios_1.default.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term=${encodeURIComponent(company.name)}[Affiliation]&retmode=json&retmax=3`);
            const pmids = searchRes.data.esearchresult.idlist || [];
            if (pmids.length > 0) {
                const summaryRes = await axios_1.default.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=${pmids.join(",")}&retmode=json`);
                ...
                for (const pmid of pmids) {
                    ...
                    await prisma.companyPublication.create({
                        data: {
                            ...
                            authors: pub.authors?.map((a) => a.name).join(", ") || "Unknown",
                            source: "PubMed",
